...
class DatabaseManager:
    ...
    def get_hourly_stats(self, days: int = 7) -> list[dict]:
        """
        Средняя посещаемость по часам суток за последние N дней.
        Возвращает 24 записи: [{"hour": 0, "avg_people": 3.2}, ...]
        """
        self._cursor.execute(
            """
            SELECT
    ...
    def get_daily_stats(self, days: int = 30) -> list[dict]:
        """
        Максимальная посещаемость и количество распознаваний по дням.
        Возвращает: [{"date": "2024-01-15", "max_people": 12, "face_events": 3}, ...]
        """
        self._cursor.execute(
            """
            SELECT
    ...
    def get_top_faces(self, limit: int = 10) -> list[dict]:
        """
        Самые часто встречающиеся лица.
        Возвращает: [{"name": "Ivan", "count": 42, "last_seen": "2024-01-15 14:32:00"}, ...]
        """
        self._cursor.execute(
            """
            SELECT
    ...
    def get_summary_stats(self) -> dict:
        """Сводная статистика для карточек на странице аналитики."""
        self._cursor.execute(
